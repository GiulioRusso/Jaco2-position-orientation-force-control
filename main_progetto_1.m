...
function [t, q, q_act] = main_progetto_1
...
% Gain
kp = 50;
K = diag(kp*[1 1 1]); % dim. 3x3 because we control only the position
...
for i=1:N
    ...
    % Joints velocities using Differential kinematics
    dq(:,i) = pinv(J(1:3,:,i))*(dpd(:,i) + K*error_pos(:,i)); % use only the Jp components because we control only the position
...
sgtitle("$\mathbf{K_{P}}$ = " + strjoin(string(kp)), 'Interpreter','latex')
