...
function [t, q, q_act] = main_progetto_2
...
start_rotation = Rot2Quat(T(1:3,1:3,n));% Desired quaternion as the starting rotation configuration to keep during the movement
...
pd  = zeros(3,N);               % Desired position e.e wrt base frame
...
wd = zeros(3,N);                % Desired angular velocity wrt e.e. frame
w = zeros(3,N);                 % Angular velocity e.e. wrt e.e. frame
quat_d = zeros(4,N);            % Desired quaternion vector e.e.
...
for i=1:N
    ...
    %   - keep constant orientation during the trajectory
    %   
    %           2 ------ 3
    %           |        |
    %           |        |
    ...
    % Evaluate the current desired quaternion (the same as the starting
    % configuration) and angular velocity
    quat_d(:,i) = start_rotation;
    % Desired angular velocity wrt e.e. frame
    wd(:,i) = zeros(3,1); % since the orientation have to be the same, the angular velocity desired is zero
