function DrawRobotImp(DH, draw_plane, rest, draw_rest, desired, draw_desired, current, draw_current)
%
% Modification of the original function DrawRobot(DH)
% Draw a robot in 3D with intermediate frames, rest, desired and current
% position of the robot in order to validate an impedance control
%
% function DrawRobotImp(DH, rest, draw_rest, desired, draw_desired, current, draw_current)
%
% input:
%       DH              dim nx4     Denavit-Hartenberg table (a, alpha, d, theta)
%       draw_plane      dim 1x1     Flag (1 = true, 0 = false) to draw the plane on the e.e.
%       rest            dim 3x1     Position vector rest point e.e.
%       draw_rest       dim 1x1     Flag (1 = true, 0 = false) to draw the rest position point
%       desired         dim 3x1     Position vector desired point e.e.
%       draw_desired    dim 1x1     Flag (1 = true, 0 = false) to draw the desired position point
%       current         dim 3x1     Position vector current point e.e.
%       draw_current    dim 1x1     Flag (1 = true, 0 = false) to draw the current position point
%
% Giulio Russo - Introduction to robotics/Sistemi robotici, 2022/2023
    ...
    if (draw_rest == 1)
        % Draw rest position
        plot3(rest(1), rest(2), rest(3), 'ro', 'MarkerSize', 10, 'MarkerFaceColor', 'r');
        grid on;
        hold on;
        text(rest(1) + 0.01, rest(2) + 0.01, rest(3) + 0.01, 'x_r', 'FontSize', 12, 'FontWeight', 'bold');
        hold on;
        % Draw the position vector of the rest position
        %plot3([0, rest(1)], [0, rest(2)], [0, rest(3)], 'b-', 'LineWidth', 2);
    end
    
    if (draw_current == 1)
        % Draw current position
        plot3(current(1), current(2), current(3), 'ro', 'MarkerSize', 10, 'MarkerFaceColor', 'r');
        grid on;
        hold on;
        text(current(1) + 0.01, current(2) + 0.01, current(3) + 0.01, 'x_e', 'FontSize', 12, 'FontWeight', 'bold');
        hold on;
        % Draw the position vector of the rest position
        %plot3([0, current(1)], [0, current(2)], [0, current(3)], 'b-', 'LineWidth', 2);
    end
    
    if (draw_desired == 1)
        % Draw desired position
        plot3(desired(1), desired(2), desired(3), 'ro', 'MarkerSize', 10, 'MarkerFaceColor', 'r');
        grid on;
        hold on;
        text(desired(1) + 0.01, desired(2) + 0.01, desired(3) + 0.01, 'x_d', 'FontSize', 12, 'FontWeight', 'bold');

        % Draw the position vector of the rest position
        %plot3([0, desired(1)], [0, desired(2)], [0, desired(3)], 'b-', 'LineWidth', 2);
    end
    
    if (draw_plane == 1)
        % Draw the plane on the rest position
        ...
        % Transform the vertices using the last frame's transformation matrix
        transformed_vertices = (T_last(1:3, 1:3) * vertices' + T_last(1:3, 4))';

        % Draw the plane
        patch(transformed_vertices(:, 1), transformed_vertices(:, 2), transformed_vertices(:, 3), 'blue', 'FaceAlpha', 0.3);
    end
end
